...
> **Golden rule:** The cardinal rule of BED — never co-locate a `.bed` file
> and a cosigner xpub of the same multisig. If an attacker finds both, they
> can decrypt the descriptor and learn your full wallet structure. See [Threat
...
> **Golden rule (repeated):** The security model requires that you never co-locate
> a `.bed` file with any cosigner xpub of the same multisig. If an attacker
> finds both, they can decrypt the `.bed` and learn the full wallet structure.
> Each `.bed` copy must live in a different physical location than the xpub
